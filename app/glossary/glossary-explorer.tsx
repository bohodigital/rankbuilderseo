...
export function GlossaryExplorer({ entries }: { entries: GlossaryEntry[] }) {
  ...
  return <section className="section shell glossary-explorer">
    <div className="search-box"><label htmlFor="glossary-search">Search the index</label><input id="glossary-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try “indexing” or “measurement”" />{query ? <button className="search-clear" type="button" onClick={() => setQuery("")} aria-label="Clear glossary search">Clear</button> : <b aria-hidden="true">⌕</b>}</div>
